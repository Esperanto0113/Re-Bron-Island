...
class MenuScene extends Phaser.Scene {
  /* ── 2. 이 메서드를 MenuScene 클래스 안에 추가 ── */
  _openDifficultyModal(W, H) {
    ...
    panel.fillStyle(0x091a10, 1);
    panel.fillRoundedRect(cX, cY, cW, cH, 20);
    panel.lineStyle(2, CONFIG.COLORS.MINT, 0.5);
    panel.strokeRoundedRect(cX, cY, cW, cH, 20);
    panel.fillStyle(CONFIG.COLORS.MINT, 0.1);
    panel.fillRoundedRect(cX, cY, cW, 72, { tl: 20, tr: 20, bl: 0, br: 0 });
    cont.add(panel);

    /* 제목 */
    cont.add(
      this.add
        .text(W / 2, cY + 36, "🎮  난이도 선택", {
          fontFamily: "Jua",
          fontSize: "22px",
          color: "#AAFFCC",
        })
    ...
    closeBtn.on("pointerout", () => closeBtn.setColor("#445544"));
    ...
    const cardH = 148;
    const cardGap = 14;
    const startCardY = cY + 88;

    diffKeys.forEach((key, i) => {
      const cfg = CONFIG.DIFFICULTY[key];
      const bx = cX + 14;
      const by = startCardY + i * (cardH + cardGap);
      const bw = cW - 28;

      /* 카드 배경 */
      const cardBg = this.add.graphics();
      const isSelected = gameState.difficulty === key.toLowerCase();
      cardBg.fillStyle(isSelected ? 0x1a4a2a : 0x0f2a18, 1);
      cardBg.fillRoundedRect(bx, by, bw, cardH, 14);
      cardBg.lineStyle(
        isSelected ? 2.5 : 1.5,
        cfg.colorInt,
        isSelected ? 0.9 : 0.35,
      );
      cardBg.strokeRoundedRect(bx, by, bw, cardH, 14);
      cont.add(cardBg);

      /* 이모지 */
      cont.add(
        this.add
          .text(bx + 36, by + cardH / 2, cfg.emoji, {
            fontSize: "36px",
          })
          .setOrigin(0.5),
      );
      ...
      /* 라벨 */
      cont.add(
        this.add.text(bx + 76, by + 22, cfg.label, {
          fontFamily: "Jua",
          fontSize: "22px",
          color: cfg.colorHex,
        }),
      ...
      /* 설명 */
      cont.add(
        this.add.text(bx + 76, by + 52, cfg.desc, {
          fontFamily: "Nunito",
          fontSize: "13px",
          color: "#88BBAA",
          lineSpacing: 4,
      ...
      /* 수치 표시 */
      const stats = [
        `🏭 오염 +${cfg.pollutionPerTick}/틱`,
        `🗑️ 더티 확률 ${Math.round(cfg.dirtyChance * 100)}%`,
        cfg.gameOver
          ? cfg.gameOverGrace > 0
            ? `⚠️ 140부터 ${cfg.gameOverGrace / 1000}초`
            : "☠️ 140부터 즉시 종료"
          : "✅ 게임오버 없음",
      ];
      stats.forEach((s, si) => {
        cont.add(
          this.add.text(bx + 76, by + 80 + si * 18, s, {
            fontFamily: "Nunito",
            fontSize: "12px",
            color:
              si === 2
      ...
      /* 클릭 히트 영역 */
      const zone = this.add
        .zone(bx + bw / 2, by + cardH / 2, bw, cardH)
        .setInteractive({ useHandCursor: true });
      cont.add(zone);

      zone.on("pointerover", () => {
        cardBg.clear();
        cardBg.fillStyle(0x1a3a22, 1);
        cardBg.fillRoundedRect(bx, by, bw, cardH, 14);
        cardBg.lineStyle(2.5, cfg.colorInt, 0.85);
        cardBg.strokeRoundedRect(bx, by, bw, cardH, 14);
      });
      zone.on("pointerout", () => {
        cardBg.clear();
        const sel = gameState.difficulty === key.toLowerCase();
        cardBg.fillStyle(sel ? 0x1a4a2a : 0x0f2a18, 1);
        cardBg.fillRoundedRect(bx, by, bw, cardH, 14);
        cardBg.lineStyle(sel ? 2.5 : 1.5, cfg.colorInt, sel ? 0.9 : 0.35);
        cardBg.strokeRoundedRect(bx, by, bw, cardH, 14);
      });
      zone.on("pointerdown", () => {
        gameState.difficulty = key.toLowerCase();
        cont.destroy();
        this._activeModal = null;
        /* 난이도 선택 후 실제 게임 시작 */
        this.cameras.main.fadeOut(500, 0, 10, 5);
        this.time.delayedCall(500, () => {
          gameState.newGame();
          gameState.difficulty = key.toLowerCase(); // newGame이 reset하므로 재설정
    ...
    /* 등장 애니메이션 */
    cont.setAlpha(0);
    this.tweens.add({ targets: cont, alpha: 1, duration: 220, ease: "Power2" });
    this._activeModal = cont;
  }
  constructor() {
    super({ key: "MenuScene" });
    this._activeModal = null;
  ...
  _drawIsland(W, H) {
    ...
    const heaps = [
      ...
      [W * 0.5, iy - 23, 72, 32, 0x2a3a5a, 0.8],
    ...
    // 쓰레기 조각
    const pieces = [
      [W * 0.24, iy - 13, 0x4fc3f7],
      [W * 0.34, iy - 9, 0xffcc80],
      [W * 0.54, iy - 19, 0xff6b6b],
      [W * 0.64, iy - 11, 0xb0bec5],
      [W * 0.74, iy - 15, 0x80deea],
      [W * 0.41, iy - 26, 0xffe66d],
      [W * 0.57, iy - 23, 0x4fc3f7],
      [W * 0.29, iy - 21, 0xa5d6a7],
    ];
    ...
    g.fillStyle(0x445555, 0.1);
    g.fillEllipse(W * 0.6, iy - 50, 62, 24);
  }
  ...
  _deadTree(g, x, y, h) {
    ...
    g.beginPath();
    g.moveTo(x, y - h * 0.55);
    g.lineTo(x + 14, y - h * 0.85);
    g.strokePath();
    g.beginPath();
    g.moveTo(x, y - h * 0.38);
    g.lineTo(x - 10, y - h * 0.54);
    g.strokePath();
  }
  ...
  _drawTrashFloating(W, H) {
    ...
    const oy = H * 0.5;
    const items = [
      [W * 0.08, oy + 30, 0x4fc3f7, 18, 8],
      [W * 0.22, oy + 55, 0xffcc80, 12, 6],
      [W * 0.78, oy + 40, 0xb0bec5, 16, 7],
      [W * 0.88, oy + 65, 0xff6b6b, 10, 5],
      [W * 0.14, oy + 90, 0x80deea, 20, 8],
      [W * 0.86, oy + 95, 0xffe66d, 14, 6],
      [W * 0.05, oy + 125, 0x4fc3f7, 11, 5],
      [W * 0.93, oy + 132, 0xa5d6a7, 15, 6],
    ];
    items.forEach(([x, y, c, rw, rh]) => {
      g.fillStyle(c, 0.42);
      g.fillEllipse(x, y, rw, rh);
    });
  }
  ...
  ══════════════════════════════════════ */
  _buildTitle(W, H) {
    this._badge = this.add
      .text(W / 2, H * 0.14, "🌏  태평양 · 플라스틱 아일랜드", {
        fontFamily: "Nunito",
        fontSize: "12px",
        color: "#88CCAA",
        fontStyle: "bold",
        letterSpacing: 2,
        backgroundColor: "#091A10DD",
        padding: { x: 12, y: 5 },
      })
      .setOrigin(0.5)
      .setAlpha(0);

    this._title1 = this.add
      .text(W / 2, H * 0.25, "Re:Born", {
        fontFamily: "Jua",
        fontSize: "66px",
        color: "#FFFFFF",
        stroke: "#00C896",
    ...
    this._title2 = this.add
      .text(W / 2, H * 0.37, "Island", {
        fontFamily: "Jua",
        fontSize: "50px",
        color: "#00E8A8",
        stroke: "#003322",
    ...
    this._sub = this.add
      .text(W / 2, H * 0.45, "쓰레기 섬을 낙원으로 되돌리는 프로젝트", {
        fontFamily: "Nunito",
        fontSize: "13px",
        color: "#77CCAA",
        fontStyle: "italic",
  ...
  _buildButtons(W, H) {
    ...
    this._btnGroup.push(
      this._outlineBtn(W / 2, baseY, 210, 50, "🎬  시나리오", "#4FC3F7", () =>
        this._openScenarioModal(W, H),
      ),
    );
    this._btnGroup.push(
      this._outlineBtn(
        ...
        "📖  게임 방법",
        "#FFE66D",
        () => this._openInstructionModal(W, H),
      ),
    );
    this._btnGroup.push(
      this._filledBtn(
  ...
  _animateTitle() {
    ...
    this.time.delayedCall(1300, () => {
      this.tweens.add({
        targets: [this._title1, this._title2],
        y: "+=9",
        duration: 2600,
        yoyo: true,
        repeat: -1,
  ...
  _openScenarioModal(W, H) {
    ...
    const slides = [
      ...
      },
      {
        emoji: "🗑️",
        title: "인류가 버린 것들",
        body: "매년 800만 톤의 플라스틱이\n바다로 흘러들어 옵니다.\n\n이 섬은 지금 이 순간에도\n조용히 커지고 있습니다.",
        color: "#FF8888",
      },
      {
        emoji: "🐠",
        title: "죽어가는 생태계",
        body: "100만 마리 이상의 바닷새와\n10만 마리의 해양 포유류가\n매년 플라스틱으로 희생됩니다.\n\n인간의 선택이 만든 재앙.",
        color: "#FFD93D",
      },
      {
        emoji: "🌱",
        title: "당신의 임무",
        body: '당신은 "Re:Born 프로젝트"의\n탐사대원으로 파견됩니다.\n\n쓰레기를 자원으로 바꾸고\n업사이클 건축물을 지어\n\n플라스틱 아일랜드를\n되살리세요!',
        color: "#00E8A8",
      },
    ];
  ...
  _modal(W, H, title, accentHex, slides) {
    ...
    const render = (idx) => {
      ...
      area.add(
        this.add
          .text(W / 2, sy + 20, s.emoji, { fontSize: "60px" })
          .setOrigin(0.5),
      );

      area.add(
        this.add
          .text(W / 2, sy + 100, s.title, {
            fontFamily: "Jua",
            fontSize: "20px",
            color: s.color,
            stroke: "#071007",
    ...
    /* 탭 전체 영역 → 슬라이드 전진 */
    const tap = this.add
      .zone(W / 2, cY + cH / 2 + 10, cW - 70, cH - 90)
      .setInteractive();
    tap.on("pointerdown", () => {
      if (cur < slides.length - 1) {
        cur++;
  ...
  _navBtn(container, x, y, label, colorHex, onClick) {
    ...
    bg.fillStyle(ci, 0.12);
    bg.fillCircle(x, y, 22);
    bg.lineStyle(1.5, ci, 0.45);
  ...
  _startGame() {
    /* 난이도 모달을 먼저 띄움 */
    if (this._activeModal) return;
    this._openDifficultyModal(CONFIG.WIDTH, CONFIG.HEIGHT);
  }
}
